...
class MultiheadAttention(nn.Module):
    ...
    def forward(self, key, value, query, mask=None, query_mask=None):
        ...
        self.attention = self.attn_dropout(attn).view(
            key.size(0) // 4, 4, -1, key.size(1)
        )
...
class Attention(nn.Module):
    ...
    def forward(self, memory, decoder_input, mask=None, query_mask=None):
        ...
        K = self.K(memory).view(
            batch_size, seq_k, self.num_attn_heads, self.num_hidden_per_attn
        )
        V = self.V(memory).view(
            batch_size, seq_k, self.num_attn_heads, self.num_hidden_per_attn
        )
        Q = self.Q(decoder_input).view(
            batch_size, seq_q, self.num_attn_heads, self.num_hidden_per_attn
        )
        ...
        result = result.view(
            self.num_attn_heads, batch_size, seq_q, self.num_hidden_per_attn
        )
...
class PositionWiseFeedForward(nn.Module):
    ...
    def __init__(self):
        ...
        self.w_1 = Convolution(num_hidden, num_hidden * 4, kernel_size=1, w_init="relu")
        self.w_2 = Convolution(
            num_hidden * 4, num_hidden, kernel_size=1, w_init="linear"
        )
...
class EncoderPrenet(nn.Module):
    ...
    def forward(self, input):
        ...
        for conv1d, bn, dropout in zip(
            self.conv1d_layers, self.bn_layers, self.dropout_layers
        ):
    ...
    def __init__(self, args=None):
        ...
        self.pos_emb = nn.Embedding.from_pretrained(
            self.get_sinusoid_encoding_table(
                self.enc_maxlen, self.num_hidden, padding_idx=0
            ),
            freeze=True,
        )
...
def optim4GPU(model, total_steps):
    ...
    return BertAdam(
        optimizer_grouped_parameters, lr=hp.lr, warmup=hp.warmup, t_total=total_steps
    )
        ...
        try:
            for epoch in range(hp.epochs):
                ...
                for i, data in data_iter:
                    ...
                    loss = self.criterion(
                        mask_lm_output.transpose(1, 2), data["mlm_label"]
                    )
                    ...
                    # write a log
                    post_fix = (
                        "\tEpoch:%2d, Iter:%5d, Step:%5d, AvgLoss: %.6f, Loss: %.6f"
...
def collate_mlm(batch):
    ...
    output = {"mlm_input": instrs, "mlm_label": labels, "input_position": position}
...
def train_model(vocab_path, args):
    ...
    if args.train_dataset and args.valid_dataset:
        ...
        print(
            "[+] Split the given dataset (%.2f): training (%d), validation (%d)"
...
if __name__ == "__main__":
    ...
    # Path settings
    parser.add_argument(
        "-op", "--output_path", required=False, type=str, help="path for the bert model"
    )
    parser.add_argument(
        "-vp", "--vocab_path", required=False, type=str, help="path of vocab"
    )
