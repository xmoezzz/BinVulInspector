...
#   (ACSAC 38th Annual Computer Security Applications Conference)#
#                                                              #
#  Original Authors:                                            #
#    Sunwoo Ahn <swahn@sor.snu.ac.kr>, Seoul National University#
#    Hyungjoon Koo <kevin.koo@skku.edu>, Sungkyunkwan University#
#                                                              #
#  Modified by: Eric                                         #
#                                                              #
...
from unit import BinarySummary

import normalize_ghidra


def run_normalization(target, normalization_level=3):
    # [Step 4] Generate data for machine learning
    BS, corpus_ctr, corpus_data, corpus_voca = normalize_ghidra.generate_learning_data(
        target + ".json", True
    )
...
def corpus_generator(
    target, pkl_dir, dmp_level=None, write_json=False, normalization_level=3
):
    ...
    if not write_json and not dmp_level and os.path.isfile(pkl_dmp_path):
        ...
        try:
            ...
            print("\t[-] Failed %s..." % pkl_dmp_path, err)
    else:
        print("\t[+] Generating %s..." % pkl_dmp_path)
        BS, corpus_ctr, corpus_data, corpus_voca = run_normalization(
            target, normalization_level=normalization_level
        )
...
class CorpusGenerator:
        ...
        if os.path.isfile(self.target):
            ...
            for ea in os.listdir(target_dir):
                ...
                # The following condition might be changed upon dataset names
                    ...
                    ida_dmp_path = target_path + ".json"

                    if not os.path.exists(ida_dmp_path):
                        print(
                            "[-] Terminating: %s has not been found!" % (ida_dmp_path)
                        )
                        sys.exit(-1)
                    else:
    ...
    def run_slow(
        self, pkl_dir, dmp_level=None, write_json=False, normalization_level=3
    ):
        for target in sorted(self.targets):
            corpus_info = corpus_generator(
                target,
                pkl_dir,
                dmp_level=dmp_level,
    ...
    def run_fast(self, pkl_dir, num_workers=4):
        """
        ...
        try:
            ...
            corpora_info = pool.map(
                functools.partial(corpus_generator, pkl_dir=pkl_dir), self.targets
            )
            pool.terminate()
...
def training_corpus_generator(binary_dir, corpus_dir, pkl_dir, num_workers=None):
    ...
    if num_workers:
        DG.run_fast(pkl_dir, num_workers=num_workers)
    else:
    ...
    corpus_fp = os.path.join(
        corpus_dir, "pretrain." + os.path.basename(binary_dir) + ".corpus.txt"
    )
    voca_fp = os.path.join(
        corpus_dir, "pretrain." + os.path.basename(binary_dir) + ".corpus.voca.txt"
    )
...
def binsim_corpus_generator(binary_path, result_corpus_fp, pkl_dir):
    ...
    max_opt = 0
    for compiler, optlevel, arch in product(["clang", "gcc"], ["O0", "O1", "O2", "O3"], ["arm", "mips", "x86", "x64", "riscv64"]):
        l = compiler + optlevel + arch
        label_pos[l] = max_opt
        pos_label[max_opt] = l
        max_opt += 1
    max_opt -= 1
    ...
    if os.path.isdir(binary_path):
        ...
        for target in bar:
            ...
            try:
                BS: BinarySummary = pickle.load(open(pkl_dmp_path, "rb"))
            except EOFError:
                logging.info("[-] Got EOFError when loading %s" % (pkl_dmp_path))
                continue
            bin_name = BS.bin_name.rsplit('-', 3)[0]
            label = BS.compiler + BS.opt_level + BS.arch
            ...
            for fs in BS.fns_summaries:
                ...
                try:
                    ...
                    if len(normalized_instrs) > 256:
                        normalized_instrs = normalized_instrs[:256]
    ...
    with open(result_corpus_fp, "w") as f:
        ...
        for idx, func_name in bar1:
            ...
            for func1, func2 in combinations(range(len(binary_funcs)), 2):
                ...
                if 5 < len(nis1.split(",")) and 5 < len(nis2.split(",")):
                    if nis1 == nis2:
                        same_nis_ctr += 1
        ...
        bar2 = util.ProgressBar(
            num_paired_corpus, name="Negative sampling: %d" % num_paired_corpus
        )
        num_diff_corpus = 0
        num_fns = len(corpus)
        ...
        while num_diff_corpus < num_paired_corpus:
            ...
            any_opt1, any_opt2 = random.randint(0, max_opt), random.randint(0, max_opt)
            ...
            if (
                ...
                continue
            if not (5 < len(nis1.split(",")) and 5 < len(nis2.split(","))):
                continue
...
def cve_corpus_generator(corpus_path, result_corpus_fp, pkl_dir):
    ...
    if os.path.isdir(corpus_path):
        ...
        query_path = (
            corpus_path[:-1] + "_strip"
            if corpus_path.endswith("/")
            else corpus_path + "_strip"
        )
        query_pkl_dir = (
            pkl_dir[:-1] + "_strip" if pkl_dir.endswith("/") else pkl_dir + "_strip"
        )
                ...
                try:
                    ...
                    if fs.fn_name in prj_vul_func_name:
                        if identifier not in corpus[prj_name]:
                            corpus[prj_name][identifier] = [("", "", "")] * len(
                                label_pos
                            )
                        corpus[prj_name][identifier][label_pos[label]] = ",".join(
                            normalized_instrs
                        )

                    # to get ground truth in stripped binaries (address is same with unstripped one)
                    strip_identifier = (
                        bin_name + "_" + str(label) + "_" + hex(fs.fn_start)
                    )
        ...
        for target in bar2:
            ...
            for fs in BS.fns_summaries:
                ...
                strip_identifier = bin_name + "_" + str(label) + "_" + hex(fs.fn_start)
                ...
                try:
                    ...
                    if identifier not in query[prj_name]:
                        query[prj_name][identifier] = [("", "", "")] * len(label_pos)
                    normalized_instrs = [x for x in filter(None, fs.normalized_instrs)]
                    query[prj_name][identifier][label_pos[label]] = ",".join(
                        normalized_instrs
                    )
...
def corpus_split(corpus_fp, type="pretrained", ratio=(0.9, 0.05, 0.05)):
    ...
    common = (
        os.path.basename(corpus_fp).split(".")[0]
        if type == "pretrained"
        else "binsim." + os.path.basename(corpus_fp).split(".")[1]
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-f", "--target_file", required=False, type=str, help="Target binary"
    )
    parser.add_argument(
        "-pkl", "--pkl_dir", required=False, type=str, help="pkl directory for corpus"
    )
    parser.add_argument(
        "-o",
        "--corpus_dir",
    ...
    parser.add_argument(
        "-w", "--num_workers", type=int, help="Number of workers for multiprocessing"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="pretrained",
        help="Support type for splitting: pretrained, binsimtask" "with (-p)",
    )
    ...
    # Note that num_workers should be given for multiprocessing
        ...
        args.train
        and os.path.exists(args.corpus_dir)
        and os.path.exists(args.binary_dir)
        and os.path.exists(args.pkl_dir)
    ):
        corpus_dir = (
            args.corpus_dir[:-1] if args.corpus_dir.endswith("/") else args.corpus_dir
        )
        binary_dir = (
            args.binary_dir[:-1] if args.binary_dir.endswith("/") else args.binary_dir
        )
        pkl_dir = (
            args.pkl_dir + "/" if not args.binary_dir.endswith("/") else args.pkl_dir
        )
        training_corpus_generator(
            binary_dir, corpus_dir, args.pkl_dir, num_workers=args.num_workers
        )

    # Generating a corpus for the downstream task: binary similarity
    # Line format (fn1_normalized_instrs, fn2_normalized_instrs, ground_truth, label)
    elif (
        args.binsim
        and os.path.exists(args.corpus_dir)
        and os.path.exists(args.binary_dir)
        ...
        binsim_corpus_fp = (
            corpus_dir + "/binsim." + os.path.basename(binary_dir) + ".corpus.txt"
        )
    ...
    # Compare all functions in query binaries with functions of interest (i.e., vulnerable functions)
        ...
        binsim_corpus_fp = (
            args.corpus_dir + "/cve." + os.path.basename(args.binary_dir) + "corpus.txt"
        )
    ...
    #     nn, _, _, _ = run_normalization(
    #         args.target_file, normalization_level=args.norm_level
    #     )
