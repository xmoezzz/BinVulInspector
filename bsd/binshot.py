...
class SimilarityModel(nn.Module):
    ...
    def to_emb(self, x, x_pos):
        x, x_attn_list, x_xs = self.bert(x, x_pos)
        return x[:, 0, :]
...
class SimilarityTrainer:
    def __init__(
        ...
        if ft_model_path != "":
            self.model = (
                torch.load(ft_model_path, map_location=device_loc).to(self.device)
                if torch.cuda.device_count() == 1
                else torch.load(ft_model_path).to(self.device)
            )
        else:
        ...
        try:
            for epoch in range(hp.epochs):
                ...
                for i, data in data_iter:
                    ...
                    preds_batch = (
                        torch.round(torch.sigmoid(sim_output)).cpu().detach().numpy()
                    )
                    ...
                    if i % self.log_freq == 0:
                        result_acc = compute_prediction_metric(
                            preds, labels, avg="binary"
                        )
                        post_fix = (
                            "\tIter:%5d, Step:%5d, AvgLoss: %.6f, Loss: %.6f, Acc: %.3f, "
        ...
        with torch.no_grad():
            for i, data in data_iter:
                ...
                scores = np.concatenate(
                    (
                        scores,
                        torch.sigmoid(sim_output).cpu().detach().numpy().reshape(-1),
                    ),
                    axis=-1,
                )
            ...
            post_fix = (
                "\tAcc: %.3f, Precision: %.3f, Recall: %.3f, F1: %.3f, AUC: %.3f"
                % (
                    result_acc["accuracy"],
                    result_acc["precision"],
                    result_acc["recall"],
...
class SimDataset(Dataset):
    def __init__(self, corpus_path, vocab, encoding="utf-8"):
        ...
        with open(corpus_path, "r", encoding=encoding) as f:
            for line in tqdm.tqdm(f, desc="Loading Dataset", total=self.num_data):
                ...
                len_tokens_f1 = f1_corpus.count(",") + 1
                len_tokens_f2 = f2_corpus.count(",") + 1
    ...
    @staticmethod
    def random_word(sentence: str, wv=None):
        tokens = sentence.split(",")

        voca_ins = []
        for i, insn in enumerate(tokens):
...
def run_model(
    bert_model_path: str,
    vocab_path: str,
    corpus_paths: typing.Dict[str, str],
    output_path: str,
    result_path: str,
    cuda: str,
    seed=99,
    ...
    if args.ft_model_path == "":
        ...
        train_data_loader = DataLoader(
            ...
            collate_fn=lambda batch: collate_sim(batch),
            shuffle=True,
        )
        ...
        pretrained = (
            torch.load(bert_model_path, map_location=device_loc)
            if torch.cuda.device_count() == 1
            else torch.load(bert_model_path)
        )
        pretrained.eval()
...
if __name__ == "__main__":
    ...
    # Dataset paths
    parser.add_argument(
        "-tn", "--train_dataset", required=False, type=str, help="train dataset"
    )
    parser.add_argument(
        "-vd", "--valid_dataset", required=False, type=str, help="validation dataset"
    )
    parser.add_argument(
        "-tt", "--test_dataset", required=False, type=str, help="test dataset"
    )

    # Input paths
    parser.add_argument(
        "-vp", "--vocab_path", required=False, type=str, help="vocabulary (see voca.py)"
    )
    parser.add_argument(
        "-bm", "--bert_model_path", required=False, type=str, help="pretraining model"
    )
    parser.add_argument(
        "-fm",
        "--ft_model_path",
    ...
    # Output paths
    parser.add_argument(
        "-op", "--output_path", required=False, type=str, help="output path"
    )
    parser.add_argument(
        "-r",
        "--result_path",
        ...
        type=str,
        default="",
        help="result file path for evaluation",
    )
