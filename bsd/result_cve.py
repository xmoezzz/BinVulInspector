...
class CveInfo:
    def __init__(self):
        ...
        self.program_list = {
            "libav_debug_avconv": ["decode_cell_data"],
            "ntp_debug_ntpd": ["crypto_recv", "ctl_putdata", "configure"],
            "openssl_debug_openssl": [
                "tls1_process_heartbeat",
                "dtls1_process_heartbeat",
                "dtls1_get_message_fragment",
                "OBJ_obj2txt",
...
class CveMetric(CveInfo):
    ...
    def parse(self):
        ...
        for line in tqdm(lines, desc="parsing file..."):
            ...
            if int(float(true)) == 1:
                bn_fn, corpus_comp_opt, query_comp_opt = re.split(
                    "(?<=\w):(?=\w)", file_info
                )
                query_program, query_func = bn_fn.split("@")

            elif int(float(true)) == 0:
                corpus_bn_fn, corpus_comp_opt, query_bn_fn, query_comp_opt = re.split(
                    "(?<=\w):(?=\w)", file_info
                )
            ...
            if int(float(true)) == 1:
                if query_func in self.vulnerable_funcs:
                    if pred == 1:
                        # true positive
                        self.cve_results[query_program]["vul"][query_func][
                            query_comp_opt
                        ].append(1)
                    else:
                        # true negative
                ...
                if query_func in self.vulnerable_funcs:
                    ...
                    if (
                        query_comp_opt
                        not in self.cve_results[query_program]["nonvul"][query_func]
                    ):
                        self.cve_results[query_program]["nonvul"][query_func] = (
                            defaultdict(list)
                        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-s", "--source_file", required=True, type=str, help="result file"
    )
    parser.add_argument(
        "-v", "--save_dir", required=True, type=str, help="save directory"
    )
    ...
    for p in cve_results:
        ...
        preds, labels = cve_metric.cleaning_dict(
            cve_results[p]["vul"], cve_results[p]["nonvul"]
        )
