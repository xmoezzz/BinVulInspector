...
class WordVocab(object):
    def __init__(self, voca_list):
        ...
        self.idx_special_sym = dict(
            (idx, char) for char, idx in self.special_symbol_idx.items()
        )
        print("vocab size: %d" % self.vocab_size)

    def voca_idx(self, vocas):
        if isinstance(vocas, list):
            return [self.special_symbol_idx.get(voca, self.unk_index) for voca in vocas]
        else:
...
def build_voca(voca_fp, out_fp):
    ...
    print(
        "[+] Saved all %d (normalized) instructions"
        " as a full vocabulary set into %s" % (wv.vocab_size, out_fp)
    )
